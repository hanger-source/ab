...
`fill()` and `type()` return typed `data.field`: the requested text, settled `inputValue`, `popupBacked`, detection `signals`, and `next`. Rust reads the live node after dispatch; popup-backed inputs receive a bounded settle window before the post-action observation is captured. When `next === "selectSuggestion"`, use the newly presented popup state and select the exact suggestion before continuing. A pre-action `aria-autocomplete: null` does not override this runtime outcome.
