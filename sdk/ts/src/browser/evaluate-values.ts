...
type EncodedValue =
  ...
  | { type: "number"; value: number | "-0" | "NaN" | "Infinity" | "-Infinity" }
  | { type: "bigint" | "date"; value: string }
  | { type: "regexp"; value: { pattern: string; flags: string } }
  | { type: "array" | "set"; value: EncodedValue[] }
  | { type: "map"; value: Array<[EncodedValue, EncodedValue]> }
  | { type: "object"; value: Array<[string, EncodedValue]> };

type ArgumentsEnvelope = {
  format: typeof ARGUMENTS_FORMAT;
...
export function buildEvaluateScript(fn: Function, args: unknown[]): string {
  const source = serializePageFunction(fn);
  const payload: ArgumentsEnvelope = {
...
export function deserializeEvaluateResult(value: unknown): unknown {
...
function encodeValue(
  ...
  if (ancestors.has(object)) throw serializationError(path, "circular values are not serializable");
  ...
  try {
    ...
    if (value instanceof Date) {
      if (Number.isNaN(value.getTime())) throw serializationError(path, "invalid Date is not serializable");
    ...
    if (Object.getPrototypeOf(value) !== Object.prototype) {
      throw serializationError(path, `${(value as { constructor?: { name?: string } }).constructor?.name ?? "custom object"} is not serializable`);
    }
    rejectEnumerableSymbols(value, path);
    const descriptors = Object.getOwnPropertyDescriptors(value);
    const entries = Object.keys(value).map((key): [string, EncodedValue] => {
...
function decodeValue(value: unknown, path: string, depth: number): unknown {
  ...
  switch (encoded.type) {
    ...
    case "bigint":
      try { return BigInt(requirePrimitive(encoded.value, "string", path)); }
      catch { throw protocolError(`invalid bigint at ${path}`); }
    case "date": {
      const text = requirePrimitive(encoded.value, "string", path);
      const date = new Date(text);
      if (Number.isNaN(date.getTime()) || date.toISOString() !== text) throw protocolError(`invalid date at ${path}`);
      return date;
    }
    case "regexp":
      try { return new RegExp(encoded.value.pattern, encoded.value.flags); }
      catch { throw protocolError(`invalid regexp at ${path}`); }
    ...
    case "object": {
      ...
      for (const [key, item] of encoded.value) {
        if (keys.has(key)) throw protocolError(`duplicate object key ${key} at ${path}`);
        keys.add(key);
...
function pageEvaluate(fn: (...args: unknown[]) => unknown, payload: ArgumentsEnvelope): Promise<ResultEnvelope> {
  const maxDepth = 100;
  const decode = (value: EncodedValue, depth: number): unknown => {
    if (depth > maxDepth) throw new Error("AB evaluate argument exceeds maximum depth");
    switch (value.type) {
      case "undefined": return undefined;
      case "null": return null;
      case "string":
  ...
  const encode = (value: unknown, ancestors: Set<object>, depth: number): EncodedValue => {
    if (depth > maxDepth) throw new Error("AB evaluate result exceeds maximum depth");
    if (value === null) return { type: "null" };
    ...
    if (typeof value !== "object") throw new Error(`${typeof value} result values are not serializable`);
    if (ancestors.has(value)) throw new Error("circular result values are not serializable");
    ancestors.add(value);
    try {
      if (Array.isArray(value)) {
        for (let index = 0; index < value.length; index += 1) {
          if (!Object.prototype.hasOwnProperty.call(value, index)) throw new Error("sparse result arrays are not serializable");
        }
      ...
      if (value instanceof Map) return { type: "map", value: [...value].map(([key, item]) => [encode(key, ancestors, depth + 1), encode(item, ancestors, depth + 1)]) };
      if (value instanceof Set) return { type: "set", value: [...value].map((item) => encode(item, ancestors, depth + 1)) };
      if (Object.getPrototypeOf(value) !== Object.prototype) throw new Error("custom result objects are not serializable");
      const descriptors = Object.getOwnPropertyDescriptors(value);
      return {
        type: "object",
        value: Object.keys(value).map((key) => {
          const descriptor = descriptors[key];
          if (!descriptor || !("value" in descriptor)) throw new Error("accessor result properties are not serializable");
...
function serializePageFunction(fn: Function): string {
  const source = Function.prototype.toString.call(fn).trim();
  if (!source || source.includes("[native code]")) throw serializationError("$function", "a user-defined function is required");
  if (isStandaloneFunctionSource(source)) return `(${source})`;
...
function isStandaloneFunctionSource(source: string): boolean {
  if (/^(?:async\s+)?function(?:\s*\*)?\b/.test(source)) return true;
  if (/^\(/.test(source) || /^async\s*\(/.test(source)) return true;
  return /^(?:async\s+)?[$A-Z_a-z][$\w]*\s*=>/.test(source);
}

function encodeNumber(value: number): number | "-0" | "NaN" | "Infinity" | "-Infinity" {
...
function rejectArrayExtras(value: unknown[], path: string): void {
  const extra = Object.keys(value).find((key) => !/^\d+$/.test(key) || Number(key) >= value.length);
  if (extra !== undefined) throw serializationError(`${path}.${extra}`, "extra array properties are not serializable");
  rejectEnumerableSymbols(value, path);
}

function rejectEnumerableProperties(value: object, path: string): void {
  if (Object.keys(value).length) throw serializationError(path, "extra enumerable properties are not serializable");
  rejectEnumerableSymbols(value, path);
}

function rejectEnumerableSymbols(value: object, path: string): void {
...
function requirePrimitive<T extends "string" | "boolean">(
  value: unknown,
  expected: T,
  path: string,
): T extends "string" ? string : boolean {
  if (typeof value !== expected) throw protocolError(`expected ${expected} at ${path}`);
  return value as T extends "string" ? string : boolean;
}

function assertDepth(depth: number, path: string, kind: string): void {
  if (depth > MAX_DEPTH) {
    throw new ABError({ kind, stage: "sdk.evaluate.value", message: `value exceeds maximum depth ${MAX_DEPTH} at ${path}` });
  }
}

function serializationError(path: string, reason: string): ABError {
  return new ABError({
...
function protocolError(message: string): ABError {
