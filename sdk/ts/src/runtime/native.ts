...
import { dataDirectory, runtimeDirectory, startupPath } from "./paths.js";
...
export function launchRuntime(binaryPath: string, provider: BrowserProvider): void {
  ...
  try {
    const env: NodeJS.ProcessEnv = {
      ...process.env,
      AB_RUNTIME_DIR: runtimeDirectory(provider),
    };
    if (provider.kind === "external") {
      env.AB_CHROME_WS_URL = provider.webSocketUrl;
    } else {
      delete env.AB_CHROME_WS_URL;
    }
...
): Promise<RuntimeStartupState | null> {
  try {
    const value = JSON.parse(await readFile(startupPath(provider), "utf8")) as RuntimeStartupState;
