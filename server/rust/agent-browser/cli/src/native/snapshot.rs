...
const LIVE_REGION_ROLES: &[&str] = &["alert", "log", "marquee", "status", "timer"];

/// Convert browser accessibility text into the text presented to an Agent.
///
/// Icon fonts commonly expose Private Use Area glyphs as part of an accessible
/// name even though the user only sees the adjacent label. Those glyphs are not
/// stable language and make the rendered snapshot disagree with semantic
/// locators (for example, `\u{e605}Filters` versus `Filters`). Format controls
/// are dropped, whitespace is collapsed, and private-use glyphs act as
/// separators when they occur between meaningful text.
pub fn model_visible_text(value: &str) -> String {
...
pub struct SnapshotCapture {
    pub text: String,
    pub captured_frame_ids: HashSet<String>,
...
/// Runs the mature snapshot/ref algorithm without changing CDP domain state.
/// The caller must hold DOM and Accessibility leases for every routed session.
pub async fn take_snapshot_prepared(
    client: &CdpClient,
    session_id: &str,
    options: &SnapshotOptions,
    ref_map: &mut RefMap,
    ...
    Ok(take_snapshot_report_prepared(
        client,
        session_id,
        options,
        ref_map,
...
/// Captures the complete reachable frame tree while preserving explicit frame
/// coverage and omission diagnostics for callers that expose completeness.
pub async fn take_snapshot_report_prepared(
    client: &CdpClient,
    session_id: &str,
    options: &SnapshotOptions,
    ref_map: &mut RefMap,
    ...
    take_snapshot_report_prepared_in_scope(
        client,
        session_id,
        options,
        ref_map,
...
/// Captures a snapshot inside an explicitly resolved DOM surface.
///
/// `root_backend_ids` is the complete DOM subtree of the active surface. It is
/// resolved by the embedding runtime so this engine can retain its AX/ref
/// semantics without guessing a CSS selector or mutating the page to add one.
pub async fn take_snapshot_report_prepared_in_scope(
    client: &CdpClient,
    session_id: &str,
    options: &SnapshotOptions,
    ref_map: &mut RefMap,
    frame_id: Option<&str>,
    iframe_sessions: &HashMap<String, String>,
    root_backend_ids: Option<&HashSet<i64>>,
) -> Result<SnapshotCapture, String> {
    let mut visited_frames = HashSet::new();
    if let Some(frame_id) = frame_id {
        visited_frames.insert(frame_id.to_owned());
    }
    take_snapshot_inner(
        client,
        session_id,
...
async fn take_snapshot_inner(
    ...
    explicit_root_backend_ids: Option<&HashSet<i64>>,
) -> Result<SnapshotCapture, String> {
    if enable_domains {
        client
    ...
    if enable_domains && effective_session_id != session_id {
        let _ = client
    ...
    let scope_backend_ids = explicit_root_backend_ids.or(selector_backend_ids.as_ref());
    let in_scope = scope_backend_ids.map(|id_set| {
        let mut values = vec![false; tree_nodes.len()];
        for (idx, node) in tree_nodes.iter().enumerate() {
            values[idx] = node
                .backend_node_id
                .is_some_and(|backend_id| id_set.contains(&backend_id))
                || node.parent_idx.is_some_and(|parent| values[parent]);
        }
        values
    });
    let effective_roots = if let Some(ref in_scope) = in_scope {
        ...
        for (idx, node) in tree_nodes.iter().enumerate() {
            ...
            let parent_in_subtree = node.parent_idx.is_some_and(|pidx| in_scope[pidx]);
            if !parent_in_subtree {
                roots.push(idx);
            }
        }

        if roots.is_empty() && explicit_root_backend_ids.is_none() {
            return Err(format!(
    ...
    for (idx, node) in tree_nodes.iter().enumerate() {
        if in_scope.as_ref().is_some_and(|scope| !scope[idx]) {
            continue;
        }
    ...
    // The rendered tree and public ref metadata must use the same model-visible
    // cursor text. Attach it before constructing RefMap entries.
    for (idx, _) in &nodes_with_refs {
        if let Some(bid) = tree_nodes[*idx].backend_node_id {
            if let Some(cursor_info) = cursor_elements.get(&bid) {
                let mut cursor_info = (*cursor_info).clone();
                cursor_info.text = model_visible_text(&cursor_info.text);
                tree_nodes[*idx].cursor_info = Some(cursor_info);
            }
        }
    }
    ...
    for (idx, nth) in &nodes_with_refs {
        ...
        let display_name = if !tree_nodes[*idx].name.is_empty() {
            tree_nodes[*idx].name.as_str()
        } else {
            tree_nodes[*idx]
                .cursor_info
                .as_ref()
        ...
        ref_map.add_with_frame_display_name(
            ref_id.clone(),
    ...
    for &root_idx in &effective_roots {
        render_tree(&tree_nodes, root_idx, 0, &mut output, options, false);
    }

    let mut captured_frame_ids = frame_id
        .map(|frame_id| HashSet::from([frame_id.to_owned()]))
        .unwrap_or_default();
    ...
    for node in tree_nodes.iter() {
        ...
        let Some(bid) = node.backend_node_id else {
            gaps.push(SnapshotGap {
                frame_id: None,
                session_id: effective_session_id.to_owned(),
                stage: "frame.owner".to_owned(),
                message: "iframe AX node has no backend node identity".to_owned(),
            });
            continue;
        };
        let ref_id = node.ref_id.as_deref().unwrap_or("");
        let child_fid = match resolve_iframe_frame_id(client, effective_session_id, bid).await {
            Ok(frame_id) => frame_id,
            Err(message) => {
                gaps.push(SnapshotGap {
                    frame_id: None,
                    session_id: effective_session_id.to_owned(),
                    stage: "frame.resolve".to_owned(),
                    message,
                });
                continue;
            }
        ...
        match Box::pin(take_snapshot_inner(
            client,
        ...
        {
            Ok(child) => {
                captured_frame_ids.extend(child.captured_frame_ids);
                gaps.extend(child.gaps);
                if !child.text.is_empty()
                    && child.text != "(empty page)"
                    && child.text != "(no interactive elements)"
                {
                    iframe_snapshots.push((ref_id.to_string(), child.text));
                }
            }
            Err(message) => gaps.push(SnapshotGap {
                frame_id: Some(child_fid),
                session_id: effective_session_id.to_owned(),
                stage: "ax.capture".to_owned(),
                message,
            }),
        }
    }

    // Insert each child snapshot after its owning Iframe line.
    ...
    if trimmed.is_empty() {
        let text = if options.interactive {
...
fn build_tree(nodes: &[AXNode]) -> (Vec<TreeNode>, Vec<usize>) {
    ...
    for (i, node) in nodes.iter().enumerate() {
        ...
        let name = model_visible_text(&extract_ax_string(&node.name));
        let value_text = extract_ax_string_opt(&node.value);

        let (level, checked, expanded, selected, disabled, required, live) =
...
fn render_tree(
    ...
    output: &mut String,
    options: &SnapshotOptions,
    inside_live_region: bool,
) {
    let node = &nodes[idx];
    let live_region_root = is_live_region(node);
    let render_live_region = inside_live_region || live_region_root;
    ...
    if node.role.is_empty()
        || (node.role == "generic"
            ...
            && node.children.len() <= 1
            && !live_region_root)
        || (node.role == "StaticText" && node.name.is_empty())
    {
        // Ignored node -- still render children
        for &child in &node.children {
            render_tree(nodes, child, indent, output, options, render_live_region);
        }
        return;
    }
    ...
    if options.interactive && !node.has_ref && !render_live_region {
        // In interactive mode, skip non-interactive but render children
        for &child in &node.children {
            render_tree(nodes, child, indent, output, options, false);
        }
        return;
    }
    ...
    if let Some(ref live) = node.live {
        if live != "off" {
            attrs.push(format!("live={}", live));
        }
    }
...
fn is_live_region(node: &TreeNode) -> bool {
    match node.live.as_deref() {
        Some("off") => false,
        Some(_) => true,
        None => LIVE_REGION_ROLES.contains(&node.role.as_str()),
    }
}
...
fn compact_tree(tree: &str, interactive: bool) -> String {
    ...
    for (i, line) in lines.iter().enumerate() {
        let live_region = line.contains("live=");
        if line.contains("ref=")
            || line.contains(": ")
            || live_region
            || (!interactive && line.contains('"'))
        {
            keep[i] = true;
            // Mark ancestors
            ...
            if live_region {
                let live_indent = count_indent(line);
                for j in (i + 1)..lines.len() {
                    if count_indent(lines[j]) <= live_indent {
                        break;
                    }
                    keep[j] = true;
                }
...
fn extract_properties(props: &Option<Vec<AXProperty>>) -> NodeProperties {
    ...
    (level, checked, expanded, selected, disabled, required, live)
}
...
mod tests {
    ...
    #[test]
    fn test_compact_tree_full_keeps_static_text_and_ancestors() {
        let tree = "- generic\n  - generic\n    - StaticText \"Click the button.\"\n    - button \"Click Me!\" [ref=e1]\n";
        let result = compact_tree(tree, false);
        assert!(result.contains("StaticText \"Click the button.\""));
        assert!(result.contains("button \"Click Me!\" [ref=e1]"));
        assert!(result.starts_with("- generic"));
    }
