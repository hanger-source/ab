...
pub struct RefEntry {
    ...
    pub role: String,
    pub name: String,
    /// Model-visible name after agent-browser's cursor-interactive text fallback.
    /// Selector recovery continues to use the original accessibility name.
    pub display_name: String,
    pub nth: Option<usize>,
    pub selector: Option<String>,
    pub frame_id: Option<String>,
    /// AB exact identities disable semantic recovery after the backend node
    /// disappears. Legacy agent-browser refs keep their existing behavior.
    pub allow_semantic_fallback: bool,
}
...
impl RefMap {
    ...
    pub fn add_with_frame_display_name(
        &mut self,
        ref_id: String,
        backend_node_id: Option<i64>,
        role: &str,
    ...
    fn insert(&mut self, ref_id: String, entry: RefEntry) {
    ...
    /// Re-key a captured ref map without changing any browser identity or
    /// selector metadata. AB uses this after a same-document observation to
    /// preserve model-visible refs for nodes that retain their backend id.
    pub fn remap_ids(&self, mapping: &HashMap<String, String>) -> Self {
        let mut remapped = Self::new();
        for (ref_id, entry) in self.entries_sorted() {
            remapped.insert(mapping.get(&ref_id).cloned().unwrap_or(ref_id), entry);
        }
        let next_ref = remapped
            .map
            .keys()
            .filter_map(|ref_id| ref_id.strip_prefix('e')?.parse::<usize>().ok())
            .max()
            .unwrap_or(0)
            + 1;
        remapped.set_next_ref_num(next_ref);
        remapped
    }
...
pub async fn resolve_element_center(
    ...
    if let Some(ref_id) = parse_ref(selector_or_ref) {
        ...
        if let Some(backend_node_id) = entry.backend_node_id {
            ...
            if !entry.allow_semantic_fallback {
                return Err(format!(
                    "Stale ref: exact backend node {} is no longer actionable",
                    backend_node_id
                ));
            }
            // Legacy agent-browser refs may re-query the accessibility tree.
        }

        if !entry.allow_semantic_fallback {
            return Err("Stale ref: exact ref has no backend node identity".to_string());
        }
        // Legacy agent-browser fallback: re-query by role/name.
...
pub async fn resolve_element_object_id(
    ...
    if let Some(ref_id) = parse_ref(selector_or_ref) {
        ...
        if let Some(backend_node_id) = entry.backend_node_id {
            ...
            if !entry.allow_semantic_fallback {
                return Err(format!(
                    "Stale ref: exact backend node {} no longer resolves",
                    backend_node_id
                ));
            }
            // Legacy agent-browser refs may re-query the accessibility tree.
...
pub async fn get_element_text_content(
    client: &CdpClient,
    session_id: &str,
    ref_map: &RefMap,
    selector_or_ref: &str,
