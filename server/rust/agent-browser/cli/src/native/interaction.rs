...
pub struct ClickResult {
    ...
    pub dialog_event: Option<CdpEvent>,
...
/// Dispatches a coordinate click through the same dialog-aware pointer
/// sequence used by semantic element clicks.
pub async fn click_at(
    ...
    dispatch_click(client, session_id, &[session_id], x, y, button, click_count).await
}
...
pub async fn fill(
    ...
    let preparation: EvaluateResult = client
        .send_command_typed(
            "Runtime.callFunctionOn",
            &CallFunctionOnParams {
                // Match the mature browser automation split: text controls are
                // selected and completed through browser input; date/time/color
                // controls use their native value contract and emit input/change.
                // Readonly/disabled/non-editable controls fail before mutation.
                function_declaration: r#"function(value) {
                    ...
                    if (this.disabled || this.getAttribute('aria-disabled') === 'true')
                        return { error: 'Element is disabled and not editable' };
                    if ((tag === 'input' || tag === 'textarea') && this.readOnly)
                        return { error: 'Element is readonly and not editable' };
                    ...
                    if (tag !== 'input')
                        return { error: 'Element is not an input, textarea, or contenteditable control' };
                    ...
                    if (directValueTypes.has(type)) {
                        this.value = value;
                        if (this.value !== value)
                            return { error: 'Value is not valid for input type ' + type };
                        this.dispatchEvent(new Event('input', { bubbles: true }));
                        this.dispatchEvent(new Event('change', { bubbles: true }));
                        return { needsInput: false };
                    }
                    if (!textTypes.has(type))
                        return { error: 'Input of type ' + type + ' cannot be filled' };
                    this.select();
                    return { needsInput: true };
                }"#
                .to_string(),
        ...
        .await?;
    let result = preparation.result.value.unwrap_or(Value::Null);
...
pub async fn type_text(
    ...
    ensure_editable(client, &effective_session_id, &object_id).await?;
...
pub async fn type_text_into_active_context(
    ...
    for ch in text.chars() {
        ...
        if key_code != 0 {
            ...
            // Characters outside the mapped keyboard layout cannot carry a
            // truthful DOM key/code identity. Insert only those characters;
            // mapped printable text uses real keyDown/keyUp semantics so
            // terminal editors and other key-event consumers can observe it.
            client
                .send_command_typed::<_, Value>(
                    "Input.insertText",
                    &InsertTextParams {
...
/// Toggle an exact AB node without changing the dispatch mechanism. If the
/// coordinate click does not produce the requested state, report the failed
/// mechanical action instead of falling back to DOM `.click()`.
pub async fn check_without_dom_fallback(
    client: &CdpClient,
    session_id: &str,
    ref_map: &RefMap,
    selector_or_ref: &str,
    ...
    set_checked_without_dom_fallback(
        client,
        session_id,
        ref_map,
        selector_or_ref,
...
pub async fn uncheck_without_dom_fallback(
    client: &CdpClient,
    session_id: &str,
    ref_map: &RefMap,
    selector_or_ref: &str,
...
async fn set_checked_without_dom_fallback(
    client: &CdpClient,
    session_id: &str,
    ref_map: &RefMap,
    selector_or_ref: &str,
    ...
    let current = super::element::is_element_checked(
        client,
        session_id,
        ref_map,
        selector_or_ref,
    ...
    .await?;
    let actual = super::element::is_element_checked(
        client,
        session_id,
        ref_map,
        selector_or_ref,
    ...
    if actual != expected {
        return Err(format!(
            "Exact checkbox action completed but checked state is {}; expected {}",
            actual, expected
        ));
    }
    Ok(())
        ...
        .send_command_typed(
            ...
            &CallFunctionOnParams {
                function_declaration: r#"function() {
                    const tag = this.tagName && this.tagName.toLowerCase();
                    if (this.disabled || this.getAttribute('aria-disabled') === 'true')
                        return 'Element is disabled and not editable';
                    if ((tag === 'input' || tag === 'textarea') && this.readOnly)
                        return 'Element is readonly and not editable';
                    if (this.isContentEditable || tag === 'input' || tag === 'textarea') return null;
                    return 'Element is not an input, textarea, or contenteditable control';
                }"#
                .to_string(),
