...
pub async fn capture_screenshot_base64(
    ...
    if options.full_page {
    ...
    } else if (options.scale - 1.0).abs() > f64::EPSILON {
        let metrics: Value = client
            .send_command_no_params("Page.getLayoutMetrics", Some(session_id))
            .await?;
        let viewport = metrics
            ...
            .ok_or_else(|| "Page.getLayoutMetrics returned no visual viewport".to_string())?;
        params.clip = Some(Viewport {
            x: viewport.get("pageX").and_then(Value::as_f64).unwrap_or(0.0),
            y: viewport.get("pageY").and_then(Value::as_f64).unwrap_or(0.0),
            width: viewport
                .get("clientWidth")
