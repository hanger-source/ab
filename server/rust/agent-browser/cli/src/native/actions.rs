...
pub struct DaemonState {
    ...
    pub(crate) idle_activity: Arc<IdleActivity>,
    ...
    pub engine: String,
    /// The browser process/profile lifecycle is owned by an embedding runtime.
    /// When this is true, a lost CDP connection is a hard failure: the engine
    /// must never fall back to launching a second browser with different state.
    pub external_browser_owner: bool,
...
impl DaemonState {
    ...
    /// Attach the complete agent-browser engine state to a Chrome endpoint
    /// whose process and profile lifecycle is owned by an embedding runtime.
    pub async fn attach_cdp(&mut self, cdp: &str) -> Result<(), String> {
        self.reset_input_state();
        self.browser = Some(BrowserManager::connect_cdp(cdp).await?);
        self.external_browser_owner = true;
...
pub async fn execute_command(cmd: &Value, state: &mut DaemonState) -> Value {
    ...
    if state.external_browser_owner && needs_launch {
        return error_response(
            &id,
            "The externally managed Chrome CDP connection is unavailable; AB must reconnect its managed browser instead of launching a fallback browser",
        );
    }
...
async fn handle_wait(cmd: &Value, state: &mut DaemonState) -> Result<Value, String> {
    ...
    if let Some(load_state) = cmd.get("loadState").and_then(|v| v.as_str()) {
        let wait_until = WaitUntil::from_name(load_state);
...
async fn execute_subaction(
    ...
    let mut delegated = cmd.as_object().cloned().unwrap_or_default();
    delegated.insert("selector".to_owned(), Value::String(selector.to_owned()));
    ...
    match subaction {
        ...
        "dblclick" => handle_dblclick(&delegated, state).await,
        "type" => handle_type(&delegated, state).await,
        "press" => {
            handle_focus(&delegated, state).await?;
            handle_press(&delegated, state).await
        }
        "uncheck" => handle_uncheck(&delegated, state).await,
        "focus" => handle_focus(&delegated, state).await,
        "clear" => handle_clear(&delegated, state).await,
        "scrollintoview" => handle_scrollintoview(&delegated, state).await,
        "select" => handle_select(&delegated, state).await,
        "upload" => handle_upload(&delegated, state).await,
        "getattribute" => handle_getattribute(&delegated, state).await,
        "boundingbox" => handle_boundingbox(&delegated, state).await,
        "count" => Ok(json!({ "count": 1 })),
...
async fn handle_getbyrole(cmd: &Value, state: &mut DaemonState) -> Result<Value, String> {
    ...
    let matches = matching_ax_nodes_by_role(&ax_tree.nodes, role, name, exact);
    if cmd.get("subaction").and_then(Value::as_str) == Some("count") {
        return Ok(json!({ "count": matches.len() }));
    }
    if matches.is_empty() {
        let _ = find_ax_node_by_role(&ax_tree.nodes, role, name, exact)?;
    }
    let index = cmd.get("index").and_then(Value::as_i64);
    if index.is_none()
        && cmd.get("strict").and_then(Value::as_bool).unwrap_or(false)
        && matches.len() != 1
    {
        return Err(format!(
            "Strict locator violation: {} matched {} elements",
            build_role_selector(role, name, exact),
            matches.len()
        ));
    }
    let selected_index = match index {
        Some(value) if value < 0 => matches.len() as i64 + value,
        Some(value) => value,
        None => 0,
    };
    let (backend_node_id, actual_name) = matches
        .get(usize::try_from(selected_index).unwrap_or(usize::MAX))
        .cloned()
        .ok_or_else(|| {
            format!(
                "No element at index {} for {} ({} matched)",
                selected_index,
                build_role_selector(role, name, exact),
                matches.len()
            )
...
fn matching_ax_nodes_by_role(
    ...
    nodes
        .iter()
        .filter(|node| !node.ignored.unwrap_or(false))
        .filter_map(|node| {
            ...
            name_matches
                .then_some(node.backend_d_o_m_node_id)
                .flatten()
                .map(|backend_id| (backend_id, node_name))
        })
        .collect()
}
...
async fn handle_semantic_locator(
    ...
    let candidates = match strategy {
        "label" => r#"(() => {
            ...
            for (const label of document.querySelectorAll('label')) {
                ...
                const target = label.control || label.querySelector('input,select,textarea,button');
                if (target) result.push(target);
            }
            for (const el of document.querySelectorAll('[aria-label]')) {
                if (matches(el.getAttribute('aria-label'))) result.push(el);
            }
            for (const el of document.querySelectorAll('[aria-labelledby]')) {
                const text = el.getAttribute('aria-labelledby').split(/\s+/)
                    .map(id => document.getElementById(id)?.textContent || '').join(' ');
                if (matches(text)) result.push(el);
            }
            return result;
        })()"#
            .to_owned(),
        "placeholder" => "Array.from(document.querySelectorAll('input[placeholder],textarea[placeholder]')).filter(el => matches(el.getAttribute('placeholder')))".to_owned(),
        "alttext" => "Array.from(document.querySelectorAll('[alt]')).filter(el => matches(el.getAttribute('alt')))".to_owned(),
        "title" => "Array.from(document.querySelectorAll('[title]')).filter(el => matches(el.getAttribute('title')))".to_owned(),
        "testid" => "Array.from(document.querySelectorAll('[data-testid]')).filter(el => matches(el.getAttribute('data-testid')))".to_owned(),
        _ => "Array.from(document.querySelectorAll('body *')).filter(el => el.children.length === 0 && matches(el.textContent))".to_owned(),
    };
    let index = cmd.get("index").and_then(Value::as_i64);
    let strict = cmd.get("strict").and_then(Value::as_bool).unwrap_or(false);
    let visible = cmd.get("visible").and_then(Value::as_bool);
    let subaction = cmd
        .get("subaction")
    ...
    let query = format!(
        r#"(() => {{
            ...
            const expected = normalize({value_json});
            const matches = value => {match_expression};
            const isVisible = el => {{
                const style = getComputedStyle(el);
                const rect = el.getBoundingClientRect();
                return style.visibility !== 'hidden' && style.display !== 'none' && rect.width > 0 && rect.height > 0;
            }};
            document.querySelectorAll('[data-agent-browser-located]').forEach(el => el.removeAttribute('data-agent-browser-located'));
            let candidates = Array.from(new Set({candidates}));
            const visibility = {visibility_json};
            if (visibility !== null) candidates = candidates.filter(el => isVisible(el) === visibility);
            const count = candidates.length;
            if ({subaction_json} === 'count') return {{ count, selected: false }};
            const requested = {index_json};
            if ({strict} && requested === null && count !== 1) return {{ count, selected: false, strictViolation: true }};
            const index = requested === null ? 0 : (requested < 0 ? count + requested : requested);
            const selected = candidates[index];
            if (!selected) return {{ count, index, selected: false }};
            selected.setAttribute('data-agent-browser-located', 'true');
            return {{ count, index, selected: true }};
        }})()"#,
        match_expression = if exact {
            "normalize(value) === expected"
        } else {
            "normalize(value).toLocaleLowerCase().includes(expected.toLocaleLowerCase())"
        },
        index_json = index.map_or_else(|| "null".to_owned(), |value| value.to_string()),
        visibility_json = visible.map_or_else(|| "null".to_owned(), |value| value.to_string()),
        subaction_json = serde_json::to_string(subaction).unwrap_or_default(),
    );
    ...
    let resolution = result.result.value.unwrap_or(Value::Null);
    let count = resolution.get("count").and_then(Value::as_u64).unwrap_or(0);
    if subaction == "count" {
    ...
    {
        return Err(format!(
            "No element found by {} '{}' at index {:?} ({} matched)",
            strategy, value, index, count
        ));
    }
    ...
    let dialog_opened = action_result
        .as_ref()
        .ok()
        .and_then(|value| value.get("dialogOpened"))
        .and_then(Value::as_bool)
        .unwrap_or(false);
    if !dialog_opened {
...
async fn handle_nth(cmd: &Value, state: &mut DaemonState) -> Result<Value, String> {
    ...
    let js = format!(
        r#"(() => {{
            ...
            if (visibility !== null) els = els.filter(el => isVisible(el) === visibility);
            const count = els.length;
            if ({subaction} === 'count') return {{ count, selected: false }};
            const requested = {idx};
            if ({strict} && requested === null && count !== 1) return {{ count, selected: false, strictViolation: true }};
            const resolved = requested === null ? 0 : (requested < 0 ? count + requested : requested);
            if (resolved < 0 || resolved >= count) return {{ count, index: resolved, selected: false }};
            els[resolved].setAttribute('data-agent-browser-located', 'true');
            return {{ count, index: resolved, selected: true }};
        }})()"#,
        sel = serde_json::to_string(selector).unwrap_or_default(),
        idx = index.map_or_else(|| "null".to_owned(), |value| value.to_string()),
        visibility = visible.map_or_else(|| "null".to_owned(), |value| value.to_string()),
        subaction = serde_json::to_string(subaction).unwrap_or_default(),
    );
    ...
    {
        return Err(format!(
            "Strict locator violation: selector '{}' matched {} elements",
            selector, count
        ));
    }
    if !resolution
    ...
    {
        return Err(format!(
            "No element at index {:?} for selector '{}' ({} matched)",
...
mod tests {
    ...
    fn find_actions_help_text_matches_the_accepted_set() {
        assert_eq!(
            FIND_ACTIONS.join(", "),
            "click, dblclick, fill, type, press, check, uncheck, hover, focus, clear, scrollintoview, select, upload, text, getattribute, boundingbox, count"
        );
    }

    /// Nested actions outside the locator surface must fail before browser work.
    #[tokio::test]
    async fn execute_subaction_rejects_undocumented_actions_before_requiring_a_browser() {
        let mut state = DaemonState::new();
        assert!(state.browser.is_none());

        for bogus in ["drag", "dispatch", ""] {
            assert!(
    ...
    fn validate_find_subaction_rejects_before_any_browser_work() {
        for command in FIND_SUBACTION_COMMANDS {
            let err = validate_find_subaction(command, &json!({ "subaction": "drag" }))
                .expect_err(&format!("'{command}' must validate its find action"));
            assert_eq!(
                err,
                format!(
                    "Unknown action 'drag' for find. Valid actions: {}.",
                    FIND_ACTIONS.join(", ")
                )
            );
        }
