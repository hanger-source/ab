...
popup 的责任落在 SessionManager，而不是 ActionTransaction。第一次修正只把 Page、Runtime、Network baseline 与 resume 并发；公开 GitHub 的原生 `target="_blank"` 链接随后证明它仍不完整：pointer gate 的 `Page.addScriptToEvaluateOnNewDocument` 仍在 resume 前被单独 await，Chrome 的 paused target 因而再次停成空 URL。`write:"state"` 与 `write:"none"` 两种 action 都在后续 `tabs.list()` 超时，排除了 post-action observation 是这次死锁根因的假设。

当前 SessionManager 使用 CDP command receipt 把“已写入浏览器”和“等待响应”分开。pointer gate、active session feature、iframe auto-attach、所有 init script 的 binding/new-document script，以及 Page、Runtime、Network baseline 按确定顺序全部写入同一 session；随后写入 no-wait `Runtime.runIfWaitingForDebugger`，最后并发等待前述响应。当前 document gate、frame tree 与 init-script current evaluation 仍在恢复之后完成，session 只有全部成功才发布 `attach ready`。这个边界直接对应 Playwright 把 page 初始化 promises 与 `Runtime.runIfWaitingForDebugger` 一起提交后统一等待的语义，也保留 agent-browser 的 no-wait resume；没有让 action 猜 popup、延长 deadline 或忽略 observation 失败。
...
最终正式产物 `4a3d34554815f819` 在同一公开页面再次交替执行六次 A/B，六个 child 全部发布到 `https://github.com/yury-s`。其中一条 `observe:none` 样本的来源 tab 同时发出 Fetch：action 224ms 返回，Fetch 在 request 后 724ms 才出现首个 response/finished/failed 终态，直接证明 action 没有等待该请求；另两条无来源请求样本为 156ms/222ms。三条 `observe:state` 分别为 617ms/1191ms/1390ms，observation 全部 completed。这里比较的是同一正式产物在同一真实页面上的责任差异，不用绝对耗时阈值判定通过。
